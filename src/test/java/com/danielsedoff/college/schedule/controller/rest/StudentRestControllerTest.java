...
class StudentRestControllerTest {
    ...
    void restGetGroupTwoShouldReturnThis() throws Exception {
        mockMvc.perform(get("/students/2")).andDo(print())
                .andExpect(MockMvcResultMatchers.content().contentType("application/json"));
    }
    ...
    void restPostGroupShouldReturnThis() throws Exception {
        mockMvc.perform(post("/students").contentType(MediaType.APPLICATION_JSON).contentType("application/json"));
    }

    @Test
    void restPutGroupShouldReturnThis() throws Exception {
        mockMvc.perform(put("/students/1").contentType(MediaType.APPLICATION_JSON).contentType("application/json"));
    }
}
